...
public class MaxDistance {
    ...
    public static  int posOrder(TreeNode head, int[] record) {
        ...
        int lMax = posOrder(head.left, record); //左孩子上存在的最大距离
        int maxfromleft = record[0];//左树，长度最深那个左树或者右树--->因为父节点只能用到左右子树其中之一（所以返回最长的那个）
        int rMax = posOrder(head.right, record); //右孩子上存在的最大距离
        int maxFromRight = record[0];//右树，长度最深那个左树或者右树--->因为父节点只能用到左右子树其中之一（所以返回最长的那个）
